...
static void sbus_to_rc(volatile const uint8_t *sbus_buf, RC_ctrl_t *rc_ctrl)
{
    ...
    rc_ctrl->rc.ch[4] -= RC_CH_VALUE_OFFSET;
		...
		RC_Ctl.rc.ch0 = (sbus_buf[0]| (sbus_buf[1] << 8)) & 0x07ff; 						//!< Channel 0
		RC_Ctl.rc.ch1 = ((sbus_buf[1] >> 3) | (sbus_buf[2] << 5)) & 0x07ff; 		//!< Channel 1
		RC_Ctl.rc.ch2 = ((sbus_buf[2] >> 6) | (sbus_buf[3] << 2) | (sbus_buf[4] << 10)) & 0x07ff;//!< Channel 2
		RC_Ctl.rc.ch3 = ((sbus_buf[4] >> 1) | (sbus_buf[5] << 7)) & 0x07ff; //!< Channel 3
		RC_Ctl.rc.s1 = ((sbus_buf[5] >> 4)& 0x000C) >> 2;															//!< Switch left
		RC_Ctl.rc.s2 = ((sbus_buf[5] >> 4)& 0x0003); 																	//!< Switch right	
		RC_Ctl.mouse.x = sbus_buf[6] | (sbus_buf[7] << 8); 											//!< Mouse X axis
		RC_Ctl.mouse.y = sbus_buf[8] | (sbus_buf[9] << 8);										  //!< Mouse Y axis
		RC_Ctl.mouse.z = sbus_buf[10] | (sbus_buf[11] << 8); 										//!< Mouse Z axis
		RC_Ctl.mouse.press_l = sbus_buf[12]; 																					//!< Mouse Left Is Press ?
		RC_Ctl.mouse.press_r = sbus_buf[13]; 																					//!< Mouse Right Is Press ?
		RC_Ctl.key.buff = sbus_buf[14] | (sbus_buf[15] << 8); 
		RC_Ctl.rc.ch4   =   sbus_buf[16]      | (sbus_buf[17] << 8); 	//!< 侧键滚轮
//		failsafe_flag = 0;
